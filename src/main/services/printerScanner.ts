...
export class PrinterScanner {
  ...
  async scanPrinters(customSubnet?: string): Promise<Printer[]> {
    ...
    this.scanning = true
    try {
      const subnet = customSubnet || getSubnet(getLocalIpAddress() || '192.168.1.1')
      ...
      for (const ip of allIps) {
        ...
        printers.push({
          ...
          ip,
          port: 9100,
          name: snmp.name === 'SNMP禁用' ? ip : snmp.name,
          model: snmp.model,
          isDefault: false,
          status: 'online'
        })
  ...
  /** 刷新单个打印机的 SNMP 名称信息 */
  async updatePrinterInfo(printer: Printer): Promise<{ name: string; model: string }> {
